...
export async function createScriptTag(client){
    if(client){
       ...
       const result= await client.post({
        ...
        console.log(`Result for the rest req using shopify is: `, result);
        return result;
    }

    console.error("Could not make the rest request as client does not exist");

}

export async function getAllScriptTags(client, src){
    if(!client){
        console.error("Could not make the request because the client does not exist");
        return;
    }
...
function getAllScriptTagsUrl(shop){
    return `${getBaseUrl(shop)}/admin/api/2022-01/script_tags.json`;
}

function getScriptTagUrl(shop, id){
    return `${getBaseUrl(shop)}/admin/api/2022-01/script_tags/${id}.json`;
}
